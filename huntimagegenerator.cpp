            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (cap.valid()) {
                    ...
                    if (cap.result() != CapState::InComplete && cap.result() != CapState::Spawned) {
                         limbsVals.append("[" + FORMAT_NUMBER(cap.capshotTime()) + "]");
                    }
                    ...
                    if (cap.result() == CapState::InComplete || cap.result() == CapState::Spawned) {
                        ...
                        text =  HuntInfo::timestampToProgressString(cap.lastLimbProgressTime()) + "  (" +  (cap.result() == CapState::Capture? "Captured": "Killed") + ")";
                    }
